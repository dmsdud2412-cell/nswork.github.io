...
// 2. 탭 전환 (지점장/직원 버튼 클릭 시 실행)
function switchTab(type) {
    currentType = type;
    // index.html의 버튼 active 클래스 제어
...
function renderTable(attendance) {
    ...
    list.forEach(person => {
        ...
        for (let i = 1; i <= 31; i++) {
            ...
            // CSS의 .weekend 클래스를 적용하여 빨간 음영 표시
...
function applyFooterColor(holidays) {
    ...
    footerRows.forEach((cell, idx) => {
        // 앞쪽 정보칸 6개를 건너뛰고 7번째 td(idx 6)가 1일임
        if (idx >= 6) {
            const dayNum = idx - 5;
